...
--Disable this command when you want to run it properly
SET @isSysAdmin = 1
...
SET @checkResult=(SELECT COUNT(*) FROM sys.databases WHERE [name]=RTRIM(@databaseToCheck));
IF @checkResult<1
...
SET @checkResult=(SELECT [compatibility_level] FROM sys.databases WHERE [name]=RTRIM(@databaseToCheck));
IF @checkResult<90
...
FOR
  SELECT  s.[name] AS dbName ,
    s.database_id
  FROM    sys.databases s
    LEFT JOIN sys.dm_hadr_availability_replica_states r ON s.replica_id = r.replica_id
  WHERE   s.state_desc = 'ONLINE'
    AND s.is_read_only != 1
...
WHILE @@fetch_status = 0
  BEGIN   
   ...
   --Enable the following code for execution purposes     
